...
if (1) { #$xauth) {
  ...
  system "ffmpeg -video_size 1920x1080 -draw_mouse 0 -framerate 30 -f x11grab -i :1+0,0 -c:v libx264 -crf 18 -preset ultrafast -vf scale=1920x1080 ".shell_quote($outputfile)." -y -f pulse -i alsa_output.pci-0000_09_00.4.analog-stereo.monitor -ac 2 &";
}
if (! $conf->{'-e'}) {
  sleep 2;
